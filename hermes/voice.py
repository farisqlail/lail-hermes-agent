...
def voice_tag_instruction(settings) -> str:
    ...
    if getattr(settings, "tts_mode", "smart") != "smart":
        return ""
    return (
        "\n\nSUARA: Awali jawaban FINAL-mu dengan SATU kalimat lisan yang "
        ...
        f"5. WAJIB TEPAT SATU {VOICE_TAG_OPEN} di seluruh balasan. Bila kamu "
        "memakai alat (tool), JANGAN menulis kalimat progres seperti 'Membuka "
        "...' atau 'Sebentar' — apalagi membungkusnya dalam tag. Diam saja "
        f"selama memanggil alat; keluarkan satu-satunya {VOICE_TAG_OPEN} hanya "
        "di awal jawaban FINAL setelah semua alat selesai, berisi INTI hasilnya "
        "(mis. isi event, bukan 'kalender terbuka').\n"
        f"Contoh: {VOICE_TAG_OPEN}Hari ini ada dua agenda, sosialisasi jam 10 "
        f"dan standup jam 9 malam.{VOICE_TAG_CLOSE}\n"
        "## Agenda Hari Ini\n"
        "...isi lengkap...\n"
