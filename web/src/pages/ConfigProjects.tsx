    ...
    <div>
      ...
      {projects.length === 0 ? (
        <div style={{ padding: '24px', backgroundColor: 'var(--surface-1)', borderRadius: 'var(--r-lg)', border: '1px solid var(--border)', textAlign: 'center', color: 'var(--text-dim)' }}>
          Belum ada proyek yang didaftarkan. Gunakan tombol di atas untuk mendaftarkan proyek baru.
        </div>
      ) : (
        <div style={{ overflowX: 'auto', backgroundColor: 'var(--surface-1)', border: '1px solid var(--border)', borderRadius: 'var(--r-lg)' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 'var(--t-sm)' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)', color: 'var(--text-dim)' }}>
                <th style={{ padding: '12px 16px', fontWeight: '600' }}>Project Name</th>
                <th style={{ padding: '12px 16px', fontWeight: '600' }}>Absolute Directory Path</th>
                <th style={{ padding: '12px 16px', fontWeight: '600', width: '100px' }}>Status</th>
                <th style={{ padding: '12px 16px', fontWeight: '600', width: '150px', textAlign: 'right' }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {projects.map((p) => (
                <tr key={p.name} style={{ borderBottom: '1px solid var(--border)' }}>
                  <td style={{ padding: '12px 16px', fontWeight: '600', color: 'var(--text)', fontFamily: 'var(--font-mono)' }}>
                    @{p.name}
                  </td>
